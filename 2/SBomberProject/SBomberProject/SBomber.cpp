...
template <typename t>
void DeleteObjCommand<t>::Execute() {
...
template <typename t>
void DropBombCommand<t>::Execute() {
    if (bombsNum > 0)
    {
...
template <typename t>
void DropBombDecoratorCommand<t>::Execute() {
    if (bombsNum > 0)
    {
        //MyTools::FileLoggerSingleton::getInstance().WriteToLog(string(__FUNCTION__) + " was invoked");
        MyTools::LoggerSingleton::getInstance().WriteToLog(string(__FUNCTION__) + " was invoked");
...
template<typename t>
void SBomber::CommandExecuter(AbstractCommand<t>* pCommand) {
    pCommand->Execute();
    delete pCommand;
}
